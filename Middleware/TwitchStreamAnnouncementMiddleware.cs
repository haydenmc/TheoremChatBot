...
{
    public class TwitchStreamAnnouncementMiddleware :
        IMiddleware
    {
        /*
            sample request for getting stream information:
            curl -H "Authorization: Bearer $(curl -X POST https://id.twitch.tv/oauth2/token\?
            client_id\=jeza5r6riexor5bnxfbstoly8ksn5o\&client_secret\=7n7p8cgz25zratzfxzajmevz9eubsb\&grant_type\=client_credentials 
            | jq .access_token -cr)" https://api.twitch.tv/helix/streams/metadata\?user_login\=disi01

            sample response for offline stream:
            {"data":[],"pagination":{}}

            sample response for online stream:
            {"data":[{"user_id":"36769016","user_name":"TimTheTatman","game_id":"512710","overwatch":null,"hearthstone":null}],"pagination":{"cursor":"IA"}}

            sample response for OAuth request:
            {"access_token":"4h5sx21j89ye68nihsjhbht8gqd5g3","expires_in":5658827,"token_type":"bearer"}

            sample request for revalidation:
            curl -H "Authorization: OAuth $(curl -X POST https://id.twitch.tv/oauth2/token\?client_id\=jeza5r6riexor5bnxfbstoly8ksn5o\
            &client_secret\=7n7p8cgz25zratzfxzajmevz9eubsb\&grant_type\=client_credentials | jq .access_token -cr)" 
            https://id.twitch.tv/oauth2/validate

            sample response for revalidation:
            {"client_id":"jeza5r6riexor5bnxfbstoly8ksn5o","scopes":[],"expires_in":5175185}
        */

        /// <summary>
        /// Logging instance
        /// </summary>
        private ILogger<TwitchStreamAnnouncementMiddleware> _logger;
        ...
        /// </summary>
        private const string _restHostname = "twitch.com";
        ...
        public TwitchStreamAnnouncementMiddleware(
            ILogger<TwitchStreamAnnouncementMiddleware> logger,
            ...
            IEnumerable<IChatServiceConnection> chatServiceConnections)
        {
            ...
            _logger.LogInformation("Starting TwitchStreamAnnouncement Middleware...");
            ConnectionCredentials credentials = new ConnectionCredentials(this._clientId, this._clientSecret);
